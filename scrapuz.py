...
def insertfx():
    try:
        ...
        x = requests.get("https://www.xe.com/currencyconverter/convert/?Amount=1&From=UZS&To=USD")
        print(x.status_code)
        parsed_body=html.fromstring(x.text)
        fx = float(parsed_body.xpath("//p[contains(.,'1 USD =')]/text()")[0].replace("1 USD = ","").replace(" UZS","").replace(",",""))
        ymdstr = dt.datetime.strftime(dt.datetime.utcnow(),'%Y-%m-%d')
...
copy_to_web()
#init_sql_schema()
sendTelegram("INFO: [uz](https://www.markowitzoptimizer.pro/blog/25)\n<img src='https://www.markowitzoptimizer.pro/static/pics/svg/UZ7029000005-priceusd.svg'>")
